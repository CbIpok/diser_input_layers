#!/usr/bin/env python
"""Visualize .npy arrays with NaN interpolation, friendly RMSE titles, and optional drag & drop GUI."""
...
def _underscored_to_float(tag: str) -> float:
    return float(tag.replace("_", "."))


def _format_rmse_mean_title(stem: str) -> str | None:
    """Parse rmse_mean naming scheme into a readable title."""
    match = re.match(
        r"^rmse_mean__i_(?P<i>[0-9_]+)__func_(?P<func>.+?)__recon_sigma_(?P<sigma>[0-9_]+|none)$",
        stem,
    )
    if not match:
        return None
    ...
    i_tag = match.group("i")
    func_tag = match.group("func")
    sigma_tag = match.group("sigma")

    i_list = [s for s in i_tag.split("_") if s]
    i_part = ", ".join(i_list) if i_list else i_tag

    func_part = func_tag.replace("_", " ")
    func_match = re.match(r"functions(?:_pow(?P<pow>[0-9_]+))?$", func_tag)
    if func_match:
        pow_tag = func_match.group("pow")
        if pow_tag:
            func_part = f"basis: power {pow_tag.replace('_', '.')}"
        else:
            func_part = "basis: default functions"

    if sigma_tag == "none":
        sigma_part = "no recon smoothing"
    else:
        try:
            sigma_value = _underscored_to_float(sigma_tag)
            sigma_part = f"recon smoothing sigma={sigma_value:g}"
        except Exception:
            sigma_part = f"recon smoothing sigma={sigma_tag}"

    return f"RMSE mean grid | i={i_part} | {func_part} | {sigma_part}"


def format_title(path: Path, filled_pixels: int) -> str:
    parsed = _format_rmse_mean_title(path.stem)
    base = parsed or path.name
    return f"{base} (filled pixels: {filled_pixels})"
...
def launch_gui(default_radius: int, dpi: int, save_default: bool) -> None:
    ...
    def handle_file(path_str: str) -> None:
        ...
        title = format_title(file_path, replaced)
        path_var.set(title)
...
def run_cli(paths: Iterable[Path], radius: int, dpi: int, save_filled: bool) -> None:
    ...
    for path in paths:
        ...
        arr = load_array(path)
        filled, replaced = fill_nan_with_neighbors(arr, radius)
        title = format_title(path, replaced)
        plot_arrays(arr, filled, title, dpi)
        if save_filled:
