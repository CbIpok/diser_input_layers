...
def get_coeffs_for_point(x_sel: float, y_sel: float,
    ...
    if idxs.size == 0:
        ...
        print(f"[!] Point ({x_sel}, {y_sel}) not found. Nearest sample used: ({xs_f[idx]:.0f}, {ys_f[idx]:.0f}), index={idx}")
    else:
        idx = int(idxs[0])
...
def plot_overlayed_surfaces_3d(Z_true: np.ndarray, Z_hat: np.ndarray,
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title('Surface comparison: functions.wave vs reconstruction')
    ...
    legend_elems = [
        Patch(facecolor='gray', alpha=alpha_true, label='ground truth (functions.wave)'),
        Patch(facecolor='C0', alpha=alpha_hat, label='reconstructed'),
    ]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description='Compare surfaces and inspect point-wise MSE derived from coefficients.'
    )
    p.add_argument('--i', type=int, default=4,
                   help='Basis index i: uses coefs_process/basis_{i}.json and data/basis_{i}')
    p.add_argument('--folder', default='coefs_process',
                   help='Directory containing coefficient JSON files (default coefs_process)')
    p.add_argument('--basis-root', default='data',
                   help='Root path with basis_{i} directories (default data)')
    p.add_argument('--functions', default='data/functions.wave',
                   help='Path to the functions field (default data/functions.wave)')
    p.add_argument('--point', nargs=2, type=float, default=[100, 547],
                   help='Coordinate (x y) used to pick coefficients. Default 100 547')
    return p.parse_args()
...
def main():
    ...
    if (H_b, W_b) != (H_f, W_f):
        raise AssertionError(f'Dimension mismatch: bases: {(H_b, W_b)}, functions: {(H_f, W_f)}')

    if len(coefs) != k:
        print(f"[!] Warning: number of coefficients in JSON ({len(coefs)}) != number of basis functions ({k}). "
              f"Using min(k_json, k).")
    ...
    if not np.any(valid_mask):
        raise RuntimeError('No valid region: all values are NaN.')

    x_sel, y_sel = args.point
    c, used_idx = get_coeffs_for_point(x_sel, y_sel, xs, ys, coefs)
    if c.shape[0] != k:
        raise AssertionError(f'Coefficient vector length ({c.shape[0]}) does not match the number of basis functions ({k}).')

    Z_hat = reconstruct_from_bases(c, bases)
    mse_val = mse_on_valid_region(to_restore, Z_hat, valid_mask)
    print(f"MSE (valid region) for sample {used_idx} (~({xs[used_idx]}, {ys[used_idx]})): {mse_val:.6g}")
